...
int xv_x64_write_insn(xv_x64_ibuffer    *const buf,
  ...
  xv_x64_i stage[15] = { 0 };   /* we build the machine code here */
  unsigned index     = 0;
  ...
  if (insn->vex) {
    ...
    if (insn->rex_w
        || 0x08 & (insn->r1 | insn->index | insn->base))
      /* REX is required */
      stage[index++] = 0x40
                     | insn->rex_w            << 3
                     | (1 & insn->r1    >> 3) << 2
  ...
  /* Now encode operands. We need to choose ModR/M and SIB encodings that are
   * large enough to handle the displacement and scale/index/base registers. We
   * don't, however, need to worry about the immediate data; we assume that the
   * opcode is correctly specified. */
  /* TODO */
